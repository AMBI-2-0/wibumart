...
<style>
  ...
  position: relative;
  padding-bottom: 50px; /* Set the height of your footer */
}
</style>
    ...
    <div class="row d-flex justify-content-center align-items-center h-100">
      <div class="col-10">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
          <h3 style=" font-weight: 700; font-size: 50px; line-height: 61px; display: flex; align-items: center; font-family: 'Inter'; font-style: normal; color: #FFFFFF; text-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);">My Cart</h3>
          <!--<div>
    ...
    <div class="card-body p-4">
      <div class="row d-flex justify-content-between align-items-center">
        ...
        <div class="col-md-3 col-lg-3 col-xl-3">
          ...
          <p style="color:black;"><span class="text-muted ">Keterangan Produk: </span>{{ $item['release_date'] }}
        </div>
        <div class="col-md-3 col-lg-3 col-xl-2 d-flex">
          <button class="btn btn-link px-2" onclick="updateQuantity(this, 'decrement')">
            <i class="fas fa-minus"></i>
          </button>
          <input id="form{{ $loop->iteration }}" min="0" name="quantity" value="{{ $item['quantity'] }}" type="number"
            class="form-control form-control-sm" data-price="{{ $item['price'] }}" />
          <button class="btn btn-link px-2" onclick="updateQuantity(this, 'increment')">
            <i class="fas fa-plus"></i>
          </button>
        </div>
        <div class="col-md-3 col-lg-2 col-xl-2 offset-lg-1">
          <h5 class="mb-0">Rp <span id="price{{ $loop->iteration }}">{{ $item['price'] * $item['quantity'] }}</span></h5>
        </div>
...
<script>
  ...
  for (var i = 0; i < plusButtons.length; i++) {
    plusButtons[i].addEventListener('click', function() {
      ...
      var price = parseFloat(this.parentNode.previousElementSibling.textContent.replace('Rp ', '').replace('.', ''));
